import itertools
import sys
sys.path.append("..")
import time
from pathlib import Path
from typing import Optional, Tuple

import torch
import torch._dynamo.config
import torch._inductor.config
import argparse
from backend import LMBackend

parser = argparse.ArgumentParser(description='Your CLI description.')

parser.add_argument('--checkpoint_path', type=Path, default=Path("checkpoints/meta-Transformer/Transformer-2-7b-chat-hf/model.pth"), help='Model checkpoint path.')
parser.add_argument('--compile', action='store_true', help='Whether to compile the model.')
parser.add_argument('--batch', type=int, help='batch size')
parser.add_argument('--maxlen', type=int, help='max len')
parser.add_argument('--declen', type=int, help='decode len')
parser.add_argument('--prefixlen', type=int, help='prefix len')
parser.add_argument('--device', type=str, help='device')
args = parser.parse_args()

checkpoint_path = args.checkpoint_path
device = args.device
precision = torch.bfloat16
use_tp = False
max_seq_length = args.maxlen
max_batch_size = args.batch
prefix_len = args.prefixlen
declen = args.declen
declen_mix = args.declen + 2
warm_up = 10
T = 500

causal_mask = torch.tril(torch.ones(max_seq_length, max_seq_length, dtype=torch.bool, device=device))

llm = LMBackend(dtype=precision, device=device)
llm.load_model(checkpoint_path)
if args.compile:
    llm.compile()

with torch.device(device):
        llm.model.setup_caches(max_batch_size=1, max_seq_length=max_seq_length)


prompt = torch.randint(low=3, high=30000, size=(1, prefix_len), device=device)
input_pos = torch.arange(0, prefix_len, device=device)
cache_pos = torch.arange(0, prefix_len, device=device)
prompt_mask = causal_mask[:prefix_len][None, None, :, :]

dec = torch.randint(low=3, high=30000, size=(1, declen), device=device)
dec_pos = torch.arange(prefix_len, prefix_len + declen, device=device)
dec_cache_pos = torch.arange(prefix_len, prefix_len + declen, device=device)
dec_mask = causal_mask[prefix_len:prefix_len + declen][None, None, :, :]

decmix = torch.randint(low=3, high=30000, size=(1, declen_mix), device=device)
dec_posmix = torch.arange(prefix_len, prefix_len + declen_mix, device=device)
dec_cache_posmix = torch.arange(prefix_len, prefix_len + declen_mix, device=device)
dec_maskmix = causal_mask[prefix_len:prefix_len + declen_mix][None, None, :, :]

llm.encode(input_ids=prompt.view(1, -1), position_ids=input_pos, storage_ids=cache_pos, attention_mask=prompt_mask)
with torch.inference_mode():
        for _ in range(warm_up):
            llm.inference(input_ids=dec, position_ids=dec_pos, storage_ids=dec_cache_pos, attention_mask=dec_mask)
        torch.cuda.synchronize()
        t1 = time.perf_counter()
        for _ in range(T):
            llm.inference(input_ids=dec, position_ids=dec_pos, storage_ids=dec_cache_pos, attention_mask=dec_mask)
        torch.cuda.synchronize()
        t2 = time.perf_counter()

print("Max Length :{}, Decode Length :{}, Prefix Length :{}, inference time:{}s".format(max_seq_length, declen, prefix_len, (t2 - t1)/ T))






















